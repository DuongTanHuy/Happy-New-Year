...
(function () {
  const now = new Date();
  const targetDate = new Date(now.getFullYear(), now.getMonth(), 11);
  const secondsUntilTarget = Math.floor((targetDate - now) / 1000);

  Array.from(document.querySelectorAll(".js-increment-before")).forEach(
    function (el) {
      el.innerHTML = 9;
    }
  );
  Array.from(document.querySelectorAll(".js-increment-after")).forEach(
    function (el) {
      el.innerHTML = 8;
    }
  );
...
//     }, 1000);
//   }
//   initializeClock(new Date().getFullYear() + "/2/11");
// })();
...
const store = {
  ...
  state: {
    ...
    config: {
      quality: String(IS_HIGH_END_DEVICE ? QUALITY_HIGH : QUALITY_HIGH), // will be mirrored to a global variable named `quality` in `configDidUpdate`, for perf.
      shell: "Random",
      size: "3",
      autoLaunch: true,
      finale: false,
